...
private:
  void Init()
  {
    if (Scene::scenes.empty())
      Profiler::Error("FATAL(SceneManager): At least 1 scene has to be added - add it via Scene::AddScene(...)");
...
public:
  ...
  void insertObject(GameObject *newObject)
  {
    if (newObject == nullptr)
      Profiler::Warning("[SCENE MANAGER] Trying to insert new gameobject into scene with pointer to nullptr");
